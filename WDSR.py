...
class WSDR_A(nn.Module):
    """docstring for ClassName"""
    def __init__(self):#网络中涉及到需要进行更新的参数需要在此处进行声明
        super(WSDR_A, self).__init__()
        self.hidden1 = nn.Sequential(
            nn.Conv2d(in_channels=1, out_channels=192, kernel_size=3,padding=(1,1),stride=(1,1)),# (size-kernel_size+1)/stride
        )
        self.relu = nn.Sequential(
            nn.ReLU(),
        )
        self.hidden2 = nn.Sequential(
            nn.utils.weight_norm(nn.Conv2d(in_channels=192, out_channels=1, kernel_size=3,padding=(1,1),stride=(1,1))),#为啥kernel_size=3不padding也能跑。而且loss降不下去
        )
...
class WSDR_B(nn.Module):
    ...
    def __init__(self):#网络中涉及到需要进行更新的参数需要在此处进行声明
        ...
        self.hidden1 = nn.Sequential(
            nn.Conv2d(in_channels=1, out_channels=32, kernel_size=3,padding=(1,1),stride=(1,1)),# (size-kernel_size+1)/stride
        )
        self.hidden12 = nn.Sequential(
            nn.Conv2d(in_channels=1, out_channels=9, kernel_size=3,padding=(1,1),stride=(1,1)),# (size-kernel_size+1)/stride
        )
        self.hidden2 = nn.Sequential(
            nn.Conv2d(in_channels=32, out_channels=192, kernel_size=1,stride=(1,1)),#为啥kernel_size=3不padding也能跑。而且loss降不下去
        )
        self.relu = nn.Sequential(
            nn.ReLU(),
        )
        self.hidden3 = nn.Sequential(
            nn.Conv2d(in_channels=192, out_channels=25, kernel_size=1,stride=(1,1)),#为啥kernel_size=3不padding也能跑。而且loss降不下去
        )
        self.hidden4 = nn.Sequential(
            nn.Conv2d(in_channels=25, out_channels=9, kernel_size=3,padding=(1,1),stride=(1,1)),# (size-kernel_size+1)/stride
        )
        self.PixelShuffle =nn.Sequential(
            nn.PixelShuffle(3)
        )
        self.hidden5 = nn.Sequential(
            nn.Conv2d(in_channels=1, out_channels=1, kernel_size=5,stride=(2,2)),# (size-kernel_size+1)/stride
        )
    def forward(self, x):
        x1 = self.hidden1(x)
        x2 = self.hidden2(x1)
        ...
        x3 = self.hidden3(x3)
        x4 = self.hidden4(x3)
        x4 = self.PixelShuffle(x4)
        x = self.hidden12(x)
        xs = self.PixelShuffle(x)
        x5 = x4+xs
        x5 = self.hidden5(x5)
        return x5
...
lrScheduler = lr_scheduler.StepLR(optimizer_conv, step_size=5, gamma=0.1)#lr_scheduler.ExponentialLR(optimizer_conv,gamma=0.631)
criteria = nn.L1Loss()
