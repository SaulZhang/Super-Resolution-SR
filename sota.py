...
class Baseline(nn.Module):
    ...
    def __init__(self):#网络中涉及到需要进行更新的参数需要在此处进行声明
        ...
        self.hidden11 = nn.Sequential(
            nn.Conv2d(in_channels=1, out_channels=128, kernel_size=3,padding=(1,1),stride=(1,1)),# (size-kernel_size+1)/stride
        ...
        self.hidden12 = nn.Sequential(
            nn.Conv2d(in_channels=1, out_channels=128, kernel_size=1,stride=(1,1)),#为啥kernel_size=3不padding也能跑。而且loss降不下去
            # nn.InstanceNorm2d(num_features=32),
        ...
        self.hidden2 = nn.Sequential(
            nn.Conv2d(in_channels=256, out_channels=128, kernel_size=1,stride=(1,1)),#为啥kernel_size=3不padding也能跑。而且loss降不下去
            # nn.InstanceNorm2d(num_features=32),
            # nn.GroupNorm(num_groups=4,num_channels=16),
            # nn.BatchNorm2d(num_features=16),
            nn.ReLU(),
        ...
        self.hidden31 = nn.Sequential(
            nn.Conv2d(in_channels=128, out_channels=16, kernel_size=3,padding=(1,1),stride=(1,1)),# (size-kernel_size+1)/stride
            nn.ReLU(),
        )
        self.hidden32 = nn.Sequential(
            nn.Conv2d(in_channels=128, out_channels=16, kernel_size=1, stride=(1,1)),
            nn.ReLU(),
        )
...
lrScheduler = lr_scheduler.StepLR(optimizer_conv, step_size=5, gamma=0.1)#lr_scheduler.StepLR(optimizer_conv, step_size=5, gamma=0.1)
criteria = nn.L1Loss()
